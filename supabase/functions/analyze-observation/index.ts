...
- Sé riguroso: solo marca "Evidencia clara" si hay datos explícitos que lo demuestren.
- Marca "Evidencia parcial" si hay indicios pero no evidencia completa.
- Marca "No observable" si no hay datos suficientes en los vistazos.
- Las fortalezas deben basarse solo en lo observado, no en suposiciones.
- Las recomendaciones deben ser claras, prácticas y aplicables en el aula.
- El resumen debe ser un diagnóstico breve del estilo de enseñanza.

IMPORTANTE: Responde SOLO con un JSON válido con esta estructura exacta:
{
  "indicators": [{ "name": "string", "level": "Evidencia clara|Evidencia parcial|No observable", "detail": "string breve explicando la evidencia o su ausencia" }],
  ...
  "implementation_level": { "status": "Inicial|En desarrollo|Consolidado", "reason": "justificación breve basada en los indicadores" }
}`;
...
Analiza estos registros y genera el análisis pedagógico completo con los 19 indicadores.`;
